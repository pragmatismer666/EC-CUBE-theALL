    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $builder->add(
                ...
                [
                    ...
                    'placeholder' => 'plugin_coupon.admin.placeholder.shop',
                    'multiple' => false,
                    'expanded' => false,
                    'mapped' => true,
                    'required' => true,
            ...
            // 有効期間(TO)
            ...
            )->add('coupon_use_time', HiddenType::class, [])->add(
                'CouponDetails',
                ...
                function (FormEvent $event) {
                    ...
                    if (count($data['CouponDetails']) == 0 && $data['coupon_type'] != Coupon::ALL) {
                        $form['coupon_type']->addError(
                            new FormError(trans('plugin_coupon.admin.coupontype'))
                        );
                    }
                    ...
                    if (! empty($data['available_from_date']) && ! empty($data['available_to_date'])) {
                        ...
                        if ($fromDate->gt($toDate)) {
                            $form['available_from_date']->addError(
                                new FormError(trans('plugin_coupon.admin.avaiabledate'))
                            );
                        }
                    }
                }
            )
            ->addEventListener(FormEvents::PRE_SET_DATA, [$this, 'setShopOnPreSetData'])
            ->addEventListener(FormEvents::PRE_SUBMIT, [$this, 'setShopOnPreSubmit']);
    }

    public function setShopOnPreSetData(FormEvent $event)
    {
        /** @var Coupon $Coupon */
        ...
        if ($Member->getRole() === 'ROLE_SHOP_OWNER') {
            $Coupon->setShop($Member->getShop());
            $Coupon->setShopId($Member->getShop()->getId());
        }
        $event->setData($Coupon);
    }

    public function setShopOnPreSubmit(FormEvent $event)
    {
        /** @var Coupon $Coupon */
        $Coupon = $event->getData();
        /** @var \Eccube\Entity\Member $Member */
        $Member = $this->tokenStorage->getToken()->getUser();
        if ($Member->getRole() === 'ROLE_SHOP_OWNER') {
            $Coupon['Shop'] = $Member->getShop()->getId() ?? null;
        }
        $event->setData($Coupon);
    }
