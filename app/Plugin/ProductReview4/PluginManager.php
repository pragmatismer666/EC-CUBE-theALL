...
class PluginManager extends AbstractPluginManager
{
    ...
    public function enable(array $meta, ContainerInterface $container)
    {
        ...
        $this->createPurchasedStatus($em);
        $this->createVoteTypes($em);
        // CSV出力項目設定を追加
    ...
    protected function createPurchasedStatus(EntityManagerInterface $em)
    {
        $Status = $em->find(ProductPurchasedStatus::class, 1);
        if ($Status) {
            return;
        }

        $Status = new ProductPurchasedStatus();
        $Status->setId(ProductPurchasedStatus::NOT_PURCHASED);
        $Status->setName('購入前');
        $Status->setSortNo(1);
        ...
        $Status = new ProductPurchasedStatus();
        $Status->setId(ProductPurchasedStatus::PURCHASED);
        $Status->setName('購入確認済み');
        $Status->setSortNo(2);
    ...
    protected function createVoteTypes(EntityManagerInterface $em)
    {
        $VoteType = $em->find(VoteType::class, 1);
        if ($VoteType) {
            return;
        }

        $VoteType = new VoteType();
        $VoteType->setId(VoteType::UPVOTE);
        $VoteType->setName('役に立った');
        $VoteType->setSortNo(1);

        $em->persist($VoteType);
        $em->flush();

        $VoteType = new VoteType();
        $VoteType->setId(VoteType::DOWNVOTE);
        $VoteType->setName('役に立たなかった');
        $VoteType->setSortNo(2);
